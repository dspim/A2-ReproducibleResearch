...
3. `deployApp(appDir="appdir_name",)`

## 小挑戰11 -- 將最後的app放到shinyapps

Windows 需要較多更動
- 檔案放致在apps下面，與`ui.R`、`server.R`同目錄下
- 將`load("ubikeweather_big5.rda")` 改成`load("ubikeweather_utf8.rda")`
- download `global.R` for 編碼支援
- 程式內部除了註解不能放有中文字內容
- 此部分所需時間較久，需要上傳資料與檔案，上課操作時流量網路注意
