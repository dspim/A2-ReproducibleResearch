...
author: "George & Wush"
date: '`r Sys.Date()`</br><a href="http://shiny.rstudio.com/gallery/">shiny Gallery</a>'
#subtitle:"Shiny"
output:
...
- UI端:`tableOutput`或`dataTableOutput` <--> Server端: `renderTable` 或 `renderDataTable`
...
- shiny_v0.12: `install_github("rstudio/DT")` [DT](https://rstudio.github.io/DT/) 
```{r echo=FALSE}
if (.Platform$OS.type == "windows") load("data/ubikeweather_big5.rda") else
  load("data/ubikeweather_utf8.rda")###### 新增此行
shinyApp(
...
shinyApp(
  ui=fluidPage(
    sidebarPanel(
      selectInput(inputId='District', label='district', choices=unique(ubike$sarea)) #選我選我-行政區 中文要deploy額外處理
      ),
    mainPanel(plotOutput(outputId="plot1"))
  ),
  server = function(input, output){
...
<img src="img/Useful-Layout.png" style='max-width: 80%;max-height: 80%'>
